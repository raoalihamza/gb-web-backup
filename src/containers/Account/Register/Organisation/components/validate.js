...
const validate = (values) => {
	...
	if (!values.organisation_name) {
		errors.organisation_name = {
			fr: "Le champ du nom de l'organisation ne doit pas être vide",
			en: 'Organisation name field shouldn’t be empty',
		};
	}
	if (!values.count_of_employees) {
		errors.count_of_employees = {
			fr: "Le champ Nombre d'employés ne doit pas être vide",
			en: 'Count of employees field shouldn’t be empty',
		};
	} else if (values.count_of_employees && values.count_of_employees < 0) {
		errors.count_of_employees = {
			fr: "Le nombre d'employés n'accepte que des valeurs positives",
			en: 'Count of employees accept positive value only',
		};
	}
	if (!values.region) {
		errors.region = {
			fr: 'Le nom de la région ne doit pas être vide',
			en: 'Region name field shouldn’t be empty',
		};
	}
	...
	} else if (!validateEmail(values.emailContact)) {
		errors.emailContact = {
			fr: 'Adresse e-mail invalide',
			en: 'Invalid email address',
		};
