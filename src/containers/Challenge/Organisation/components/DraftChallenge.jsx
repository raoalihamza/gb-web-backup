	...
	const data = React.useMemo(
		() => Array.isArray(challenges)
			? challenges.filter((value) => 
				...
				return {
					name: rowData?.name || '',
					startDate: rowData?.startAt?.toDate() || '',
					endDate: rowData?.endAt?.toDate() || '',
									...
									<DataTableBody
										...
										additionalColumns={(row) =>
											(<td className="data-table-data">
												<NavLink 
													to={routes.organisation.challengeEdit.replace(':id', row.original.challengeId)} 
													className="challenge-link mr-1"
												>
													{t('global.edit')}
												</NavLink>
												<NavLink 
													to={routes.organisation.challengeInfo.replace(':id', row.original.challengeId)} 
